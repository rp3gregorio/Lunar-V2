...
Fig.~\ref{fig:posterior} makes the degeneracy quantitative. We
construct a joint posterior $P(K_d, Q_b \mid \mathrm{data})$ at
each Apollo site by combining a Gaussian likelihood
$\mathcal{L}(K_d, Q_b) \propto \exp[-\tfrac{1}{2}\,N_\mathrm{deep}\,
\mathrm{RMSE}^2(K_d, Q_b)/\sigma_\mathrm{data}^2]$ (with
$\sigma_\mathrm{data}=0.5\,\si{\kelvin}$ taken from the
within-window standard deviation of the deep sensors) with a
Gaussian prior on $Q_b$ derived from the
\citet{saito2007,nagihara2018} reanalysis ($Q_b \sim
\mathcal{N}(18, 5^2)$ for A15 and $\mathcal{N}(13, 4^2)$ for A17,
in \si{\milli\watt\per\meter\squared}). The likelihood surface
in the conduction-dominated regime depends on $K_d$ and $Q_b$
only through the ratio
$|\partial_z T| = Q_b/K_d$, so the joint posterior contracts onto
a degenerate ridge along that ratio and is broken in only the
direction informed by the prior.

The marginal posterior on $K_d$ at A17 is therefore wider than the
profile-likelihood retrieval reported in the main letter. The
shift in posterior median between the canonical
$Q_b=15\,\si{\milli\watt\per\meter\squared}$ and the reanalysis
prior is the principal driver. Reporting both numbers is more
honest than reporting only one; the inter-site contrast remains
robust across the entire posterior because the same $Q_b/K_d$
ridge constrains both sites identically.

\begin{figure}[hbt]
\centering
\includegraphics[width=0.99\linewidth]{figures/fig_kd_qb_posterior.pdf}
\caption{\textbf{Joint Bayesian posterior in $(K_d, Q_b)$ at the
two Apollo sites.} Panels (a)--(b): 2-D posterior densities with
the maximum a posteriori marker (red star) and dotted iso-ratio
lines $Q_b/K_d=\mathrm{const}$. Panels (c)--(d): the marginal
posteriors on $K_d$ (blue, left axis) and on $Q_b$ (red, right
axis). The posterior contracts onto the iso-ratio ridge along
which the equilibrium temperature profile is invariant; the prior
on $Q_b$ breaks the degeneracy in only the orthogonal
direction.}
\label{fig:posterior}
\end{figure}
...
both sites. The absolute deep RMSE between model and Diviner
brightness temperature is $12.5$\,K at A15 and $17.5$\,K at A17,
with a model warm-bias at the daytime peak of $+5.0$ and
$+9.1$\,K respectively. This residual is consistent with the
well-documented anisothermal-footprint effect
\citep{bandfield2015,hayne2017}: the GCP grid pixel mixes
sub-pixel-scale shadows, slopes, and rocks, which broaden the
distribution of effective temperatures and reduce the peak
brightness compared to a 1-D homogeneous radiative-equilibrium
model. Because the Diviner residual is a forcing-side signature
that scales linearly with surface insolation rather than with
$K_d$, it does not propagate into the deep RMSE, which is dominated
by the equilibrium gradient $Q_b/K_d$ at depths well below the
diurnal skin.
...
GCP.} Filled circles are the Diviner GCP brightness temperature
binned at $0.25$-h LST resolution within $\pm 0.5^\circ$ latitude
and $\pm 1^\circ$ longitude of each landing site; solid curves are
the modelled surface temperature with the per-site retrieved
$K_d^{*}$. Diurnal-shape agreement is excellent at both sites; the
$+5$ to $+9$\,K daytime bias is consistent with the
sub-pixel anisothermal effect \citep{bandfield2015,hayne2017}
inherent to comparing 1-D homogeneous-column predictions with
Diviner footprint-averaged brightness temperatures.}
