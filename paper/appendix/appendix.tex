...
A point estimate like ``$K_d^{*} = 4.88$'' is incomplete. We need a
...
and some are missed). For each resample we recompute $K_d^{*}$. We
do this 2000 times. The collection of 2000 $K_d^{*}$ values is the
\emph{bootstrap distribution}. The middle 95\% of that distribution
is the 95\% bootstrap confidence interval. The whole procedure is
just a way to ask the data: ``how stable is your answer under
small changes in which sensors I happened to look at?''
\end{aside}
...
generate 2000 bootstrap samples of the difference; if exactly $k$
of them are negative or zero, $p \approx k/2000$. We report
$p \approx 0.013$, meaning that roughly 1 in 80 random resamples
of the data would mimic the inter-site difference by chance alone.
Conventionally a result with $p < 0.05$ is called ``statistically
significant.''
\end{aside}
...
% --------------------------------------------------------------------
\section{What is MCMC, and why \texttt{emcee}?}
...
\begin{aside}[The \texttt{emcee} sampler]
\texttt{emcee} \citep{foreman_mackey2013emcee} is the
\emph{affine-invariant ensemble} variant: 32 walkers run together
and propose moves based on the current positions of the other
walkers in the ensemble. This handles correlated parameters (such
as our diagonally-correlated $K_d$/$Q_b$ pair) much better than a
single-walker scheme. We run for 4000 steps per walker; the
first 1000 are discarded as ``burn-in'' (the walkers haven't yet
...
\begin{aside}[MCMC and the role of \texttt{emcee}]
...
We use the affine-invariant ensemble sampler
\texttt{emcee} \citep{foreman_mackey2013emcee}, which runs 32
...
from \texttt{emcee} MCMC at the two Apollo sites.} Panels (a)--(b):
