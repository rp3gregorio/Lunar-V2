...
\title{\bfseries Phase-1 Apollo HFE Validation\\[2pt]
\Large Supporting Information --- A Beginner's Walk-through\\[2pt]
\large for the Lunar-V2 / TSUKIMI Thermal-Modelling Pipeline}
\author{R.~P.~Gregorio}
\date{\today}
...
Chapter~\ref{ch:validation}. If you are a planetary-science student
opening these methods for the first time, read in order.
...
inherit that depth as our shallow-sensor exclusion threshold and
...
\begin{aside}[An explicit $K_d$ sweep is deferred to Phase~2]
The two endpoints $3.4$ and $6.3\times10^{-3}\,\si{\watt\per\meter\per\kelvin}$
bracket a factor-of-two range, but Phase~1 only tests the two
published values. A dense $K_d$ sweep that maps RMSE$_\mathrm{depth}$
versus the deep conductivity is part of the Phase-2 regression
companion paper, where it can be performed jointly with the heat-flow
$Q_b$ retrieval to avoid the well-known $K_d/Q_b$ degeneracy in
