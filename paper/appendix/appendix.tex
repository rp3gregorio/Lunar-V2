...
\begin{itemize}\itemsep2pt
  \item Apollo~15 prefers $K_d \approx 4.86$~mW/(m\,K).
...
% =====================================================================
\chapter{Concepts and methods, in plain language}\label{ch:primer}

This chapter is a concept primer. It assumes nothing about
geophysics, statistics, or programming, and walks through every
technical idea the rest of this document will use. If a sentence
in the main letter or in a later chapter uses a word like
``bootstrap,'' ``posterior,'' ``MCMC,'' ``conductivity,'' or
``borestem,'' you can come here for a self-contained explanation
before reading on. Concepts are introduced in roughly the order
they will appear in the analysis.

% --------------------------------------------------------------------
\section{What is the lunar regolith, and why does its conductivity
matter?}

The Moon's surface is covered everywhere by a loose, broken-up
layer of rock and dust called the \emph{regolith}, formed by
billions of years of micrometeorite impacts grinding the underlying
basalt and highland rock into fragments. The regolith is typically
$\sim$5--15 m thick at the maria and tens of metres at the
highlands. Its grains are loosely packed, with lots of empty pore
space between them, which makes the regolith an extraordinarily
poor conductor of heat --- about a thousand times worse at carrying
heat than solid rock, and about a hundred thousand times worse than
metal.

The number that quantifies ``how badly the regolith conducts heat''
is the \textbf{thermal conductivity}, written $K$ and measured in
watts per metre per kelvin (W\,m$^{-1}$\,K$^{-1}$). For the deep
regolith it is roughly $K \sim 10^{-3}$\,W\,m$^{-1}$\,K$^{-1}$, but
the exact value matters enormously:

\begin{itemize}\itemsep2pt
  \item it sets how quickly the lunar interior cools;
  \item it sets the depth at which buried water-ice is stable in
        permanently shadowed polar craters;
  \item it sets how much heat a future buried instrument's
        electronics will dump back into the regolith.
\end{itemize}

The \emph{deep} value of this conductivity, which we will call
$K_d$, is what this study tries to measure independently at the
Apollo 15 and 17 sites.

% --------------------------------------------------------------------
\section{What is a Heat-Flow Experiment?}

In 1971 (Apollo 15) and 1972 (Apollo 17) the astronauts hand-drilled
two boreholes about 2.4 metres deep into the regolith and lowered
into each a thin probe carrying eight platinum thermometers at
known depths. The probes recorded the regolith temperature for the
next $\sim$5 years. The original tapes were partly lost; the
restored 1971--1977 record from \citet{nagihara2018} is what this
study uses.

\begin{key}[Why deep matters]
The Moon's surface temperature swings from 100~K at night to almost
400~K at noon every $\sim$29.5 Earth-days (one lunar day). This
huge swing only penetrates about 6~cm into the regolith before it
is smoothed out --- the deep regolith below $\sim$80~cm is
essentially at a constant temperature. That constant deep
temperature is set by the \emph{interior heat flux} from below
(call it $Q_b$) and the conductivity $K_d$. Put another way: the
deep temperature gradient $\partial T / \partial z = Q_b / K_d$.
This is the equation the whole retrieval rests on.
\end{key}

% --------------------------------------------------------------------
\section{Why the analysis uses a 1-D heat equation}

A small column of regolith of cross-section 1 m$^2$ obeys the same
energy-balance physics as any other heat-conducting solid:
the time rate of change of temperature equals (the divergence of
the heat flow) divided by (the heat capacity). Written as an
equation,
\[
\rho \, c_p \, \frac{\partial T}{\partial t}
   \;=\; \frac{\partial}{\partial z}\!\left(K \, \frac{\partial T}{\partial z}\right),
\]
where $\rho$ is the bulk density (kg\,m$^{-3}$), $c_p$ is the
specific heat (J\,kg$^{-1}$\,K$^{-1}$), $T$ is temperature, $t$ is
time, and $z$ is depth. We will not derive this here; the next
chapter does. The point is just that we have a well-posed
\emph{partial differential equation} (PDE) that, given the surface
temperature history and the bottom heat flow, predicts the
temperature at every depth and every time. Solving this PDE
numerically is what the ``Crank--Nicolson scheme'' does
(\S\ref{ch:numerics}).

% --------------------------------------------------------------------
\section{What does ``retrieving $K_d$'' mean?}

The 1-D model needs many parameters --- four for the conductivity
profile $K(T,z)$, two for the density profile, four for the
specific heat, and two more for the surface forcing. For all but
one of these we adopt the published global values from
\citet{hayne2017}. We then ask: \emph{what value of $K_d$, the
deep conductivity, makes the model best match the measured Apollo
deep temperatures at this site?} ``Best'' means: smallest
root-mean-square difference between model and data, summed over the
deep sensors. The value of $K_d$ that achieves this minimum is
called $K_d^{*}$, and is reported separately at each of the two
Apollo sites.

\begin{key}[The headline finding, in one paragraph]
At Apollo 15 the data prefer $K_d^{*} \approx 4.9$\,mW\,m$^{-1}$\,K$^{-1}$.
At Apollo 17 the data prefer $K_d^{*} \approx 11.2$\,mW\,m$^{-1}$\,K$^{-1}$.
The two sites disagree by more than a factor of two, and both
disagree with the value of 3.4\,mW\,m$^{-1}$\,K$^{-1}$ that
\citet{hayne2017} adopted as the global Moon-wide value. This is
the central claim of the letter.
\end{key}

% --------------------------------------------------------------------
\section{What is a confidence interval?}

A point estimate like ``$K_d^{*} = 4.86$'' is incomplete. We need a
\emph{range} that captures how well the data constrain the value.
There are several ways to compute such a range; the one that
appears most often in this paper is called the \emph{bootstrap
confidence interval}.

\begin{aside}[Bootstrap, in plain language]
Imagine the seven Apollo 15 deep sensors. Each gives a
temperature reading. The deep RMSE between model and data depends
on which sensors are included. To estimate ``how much would $K_d^{*}$
move if we'd happened to have a slightly different set of seven
sensors,'' we randomly resample seven sensors \emph{with
replacement} from the seven we have (so some sensors appear twice
and some are missed), and additionally jitter each sensor's depth
by a small random amount ($\pm 2.5$~cm, the documented
sensor-placement uncertainty). For each resample we recompute
$K_d^{*}$. We do this 1500 times. The collection of 1500 $K_d^{*}$
values is the \emph{bootstrap distribution}. The middle 95\% of that
distribution is the 95\% bootstrap confidence interval. The whole
procedure is just a way to ask the data: ``how stable is your answer
under small changes in which sensors I happened to look at, and in
exactly how deep each one sat?''
\end{aside}

\begin{aside}[$p$-value, in plain language]
A $p$-value answers the question ``if the data really showed no
inter-site difference, how often would I get a difference at least
as large as the one I observed?'' For our two-site contrast we
generate 1500 bootstrap samples of the difference; if exactly $k$
of them are negative or zero, $p \approx k/1500$. We report
$p \approx 0.05$, meaning that roughly 1 in 20 random resamples
of the data would mimic the inter-site difference by chance alone.
Conventionally a result with $p \le 0.05$ is called ``statistically
significant'' --- so our contrast sits right at that conventional
threshold, which is why we are careful to call it suggestive rather
than definitive.
\end{aside}

% --------------------------------------------------------------------
\section{What is Bayesian inference, and what is a posterior?}

So far the bootstrap treats $K_d$ as the only thing being measured
and assumes the basal heat flux $Q_b$ is fixed at its published
value. But $Q_b$ itself is uncertain --- the original
\citet{langseth1976} reduction has been revised by
\citet{saito2007,nagihara2018}. Bayesian inference handles this
correctly: it lets us include our prior knowledge of $Q_b$ as part
of the analysis, and quantifies what the data plus prior together
say about $K_d$.

\begin{aside}[Posterior, in plain language]
The \emph{posterior} probability density is the function
$P(K_d, Q_b \mid \mathrm{data})$ that tells us, for any
combination of $(K_d, Q_b)$, how likely those values are given
everything we know: the Apollo data, the prior expectation on
$Q_b$, and the physics of conductive heat flow. A 2-D posterior
plot looks like a topographic contour map: the peak is the
most-probable parameter combination; the inner contour encloses
roughly $\sim$32\% of the probability ($1\sigma$); the outer
contour encloses 95\%.
\end{aside}

\begin{aside}[Prior, in plain language]
A \emph{prior} encodes what we believe about a parameter
\emph{before} looking at the data. For $Q_b$ we use a Gaussian
distribution centred on the canonical Saito reanalysis value with
a width that captures the published uncertainty. The prior pulls
the posterior toward plausible $Q_b$ values; the likelihood (which
encodes how well a $(K_d, Q_b)$ pair fits the Apollo data) pulls
it toward parameter combinations that match the deep temperatures.
The posterior is what's left after both forces have acted.
\end{aside}

% --------------------------------------------------------------------
\section{What is MCMC, and how do we sample the posterior?}

Computing the posterior on a fine 2-D grid is in principle
straightforward but in practice expensive: each grid point needs
the model evaluated, and a high-resolution grid in $(K_d, Q_b)$
has hundreds of thousands of points. \emph{Markov-chain
Monte Carlo} (MCMC) is a clever way to sidestep the grid: it
draws samples directly from the posterior, with samples
proportionally more likely to land in high-probability regions
than in low-probability regions.

\begin{aside}[How MCMC actually works, intuitively]
Place a "walker" at some random starting point in the parameter
space. At each step, the walker proposes a small move; if the
posterior at the new point is higher, accept the move; if lower,
accept it with probability proportional to the height ratio. Over
many steps the walker visits each region of parameter space in
proportion to the posterior density there --- that is, it
\emph{samples from the posterior}. Many independent walkers
running in parallel explore the space faster.
\end{aside}

\begin{aside}[The ensemble sampler we use]
We use the \emph{affine-invariant ensemble} MCMC sampler of
\citet{foreman_mackey2013emcee}: 32 walkers run together and propose
moves based on the current positions of the other walkers in the
ensemble. This handles correlated parameters (such as our
diagonally-correlated $K_d$/$Q_b$ pair) much better than a
single-walker scheme. We run for 4000 steps per walker; the first
1000 are discarded as ``burn-in'' (the walkers have not yet
forgotten their random starting points). The remaining
$32 \times 3000 \approx 10^5$ samples are what define the posterior
in Fig.~\ref{fig:posterior}.
\end{aside}

% --------------------------------------------------------------------
\section{What is the $K_d/Q_b$ degeneracy, and why are the
contours diagonal?}

The deep equilibrium temperature gradient is $\partial_z T = Q_b / K_d$.
If we double both $K_d$ and $Q_b$, the gradient is unchanged, and
the deep temperatures predicted by the model are also unchanged ---
the data can't tell apart $(K_d, Q_b)$ from $(\alpha K_d, \alpha Q_b)$
for any $\alpha > 0$. This is the
\textbf{$K_d/Q_b$ degeneracy} of single-borehole inversions.

In the posterior plot it shows up as diagonal stripes
(``iso-ratio rays'' emanating from the origin) instead of circular
or elliptical blobs centred on a single point. The Gaussian prior
on $Q_b$ from the Saito reanalysis breaks the degeneracy by
truncating the posterior to a finite range along the ridge.

\begin{key}[Two consequences of the degeneracy]
The degeneracy means the \emph{absolute} retrieved value of $K_d$
at A17 (e.g.~11.2 mW\,m$^{-1}$\,K$^{-1}$) is conditional on the
adopted $Q_b$: if $Q_b$ is revised down by 30\%, the retrieved
$K_d$ is also revised down by 30\%. But the \emph{contrast}
between A15 and A17 is invariant under any global rescaling of
$Q_b$ (because both sites move along the same ridge), so the
"factor-of-two contrast" is the robust claim, while the absolute
A17 number is not.
\end{key}

% --------------------------------------------------------------------
\section{What is the borestem, and why is there a ``borestem cut''?}

The Apollo HFE probe was housed inside a hollow fibreglass tube
called the \emph{borestem}, left in place after drilling so the
hole would not collapse. The fibreglass conducts heat better than
the surrounding regolith, so heat from the lunar surface (which
swings between 100\,K and 400\,K every lunation) leaks down the
tube faster than it would otherwise diffuse through the regolith.
The result: the \emph{shallow} sensors, in the upper $\sim$80\,cm
of the column, see day-night swings that are much larger than what
the regolith alone would produce. They are
\emph{borestem-contaminated}.

For this paper, we exclude all sensors above $z = 80$\,cm from the
quantitative comparison. This is called the \emph{borestem cut}.
A key methodological point in our analysis is that this cut is
determined from the \emph{model's} predicted depth-attenuation
profile, \emph{not} from a residual-vs-depth plot of the data ---
i.e.\ we don't pick the cut where the model fits worst. That
choice avoids what is called a \emph{circular validation}: an
analysis that retunes its data-selection rule whenever the answer
moves.

% --------------------------------------------------------------------
\section{A short glossary you can refer back to}

\begin{description}\itemsep1pt
  \item[Bootstrap] A resampling procedure that estimates how
        sensitive an answer is to small changes in the input
        sample.
  \item[Confidence interval (CI)] A range that contains the true
        value of a parameter with stated probability under repeated
        sampling.
  \item[Posterior] The probability density of parameter values
        \emph{after} combining the data with prior knowledge.
  \item[Prior] The probability density of parameter values
        \emph{before} the data are looked at.
  \item[Likelihood] The probability density of the observed data,
        evaluated as a function of the parameters.
  \item[MCMC] Markov-chain Monte Carlo --- a sampling algorithm
        that draws random samples whose density approximates a
        target probability distribution.
  \item[Burn-in] The initial portion of an MCMC chain that is
        discarded because the walkers have not yet entered the
        high-probability region.
  \item[Bayes' theorem] The mathematical statement
        $\mathrm{posterior} \propto \mathrm{likelihood} \times
        \mathrm{prior}$.
  \item[Degeneracy] A direction in parameter space along which the
        data cannot distinguish parameter combinations.
  \item[Crank--Nicolson scheme] A standard implicit, second-order-
        accurate way of solving the heat equation numerically.
  \item[$Q_b$] Basal heat flux --- the constant heat-flow boundary
        condition imposed at the bottom of the modelled column,
        physically representing the heat coming up from the lunar
        interior.
\end{description}

% =====================================================================
\chapter{The whole model in one read-through}\label{ch:walkthrough}

The previous two chapters told you \emph{what} the study did and
defined the vocabulary. This chapter walks the entire model from
start to finish in one continuous narrative, so that by the end you
can picture every step and know which later chapter to open for the
mathematical detail. Read it slowly once; everything here is
expanded with equations in Chapters~\ref{ch:moon}--\ref{ch:numerics}.

\section{The one question the model answers}

Strip the study to its core and it asks a single question:

\begin{quote}
\emph{Given the temperatures that the Apollo thermometers actually
recorded below ground, what value of the deep regolith thermal
conductivity $K_d$ makes a physics-based model reproduce those
temperatures best?}
\end{quote}

Everything else --- the heat equation, the solar geometry, the
bootstrap, the Bayesian posterior --- exists only to answer that one
question well and to attach an honest uncertainty to the answer.

\begin{aside}[Why $K_d$ and not something else?]
The Moon's near-surface is a layered insulator: a fluffy, very-low-
conductivity skin in the top few centimetres, grading into denser,
more conductive material with depth. The deep value $K_d$ is the
single number that controls how steeply temperature rises with depth
once you are below that transition. It sets the subsurface
equilibrium temperature, and therefore the depth at which polar ice
is stable and the heat budget of any buried instrument. It is the
quantity worth measuring.
\end{aside}

\section{Step 1 --- The forward model: from sunlight to a temperature profile}

A \emph{forward model} takes physical inputs and predicts an
observable. Ours takes (i) the position of the Sun over time, (ii) a
description of how the regolith conducts and stores heat, and (iii)
the heat leaking up from the deep interior, and predicts the
temperature at every depth and every moment.

The engine is the \emph{one-dimensional heat equation}
(Chapter~\ref{ch:pde}). ``One-dimensional'' means we track
temperature only as a function of depth $z$ and time $t$, ignoring
horizontal variation --- a good approximation because the Apollo
sites are flat and the relevant physics is vertical. The equation is
a statement of energy conservation: the rate at which a thin slab of
regolith heats up equals the difference between the heat flowing in
from above and the heat flowing out below.

Two things must be supplied at the boundaries of the modelled
column:

\begin{description}
\item[At the surface ($z=0$):] the regolith is in radiative balance.
  It absorbs sunlight, re-emits thermal infrared according to the
  Stefan--Boltzmann law, and exchanges conductive heat with the
  layer just beneath. The Sun's contribution is computed from real
  ephemeris geometry (\S\ref{sec:lst}) rather than an idealised
  sine wave, because the true local-solar-time drift over a lunation
  matters at the precision we need.
\item[At the bottom ($z=z_\mathrm{max}=5$\,m):] a fixed heat flux
  $Q_b$ flows upward out of the deep interior. This is the
  \emph{geothermal} or \emph{basal} flux. Imposing a flux (not a
  fixed temperature) is what lets the model produce a realistic
  deep temperature gradient.
\end{description}

\begin{aside}[Why the bottom boundary is a flux, not a temperature]
If we fixed the bottom temperature, we would be assuming the answer
we are trying to measure. Fixing the upward heat \emph{flux}
instead lets the temperature gradient emerge from the physics:
steeper if the regolith conducts poorly, gentler if it conducts
well. That gradient is exactly the signal $K_d$ controls.
\end{aside}

\section{Step 2 --- The regolith recipe: what the parameters mean}

The forward model needs to know, at every depth and temperature,
three material properties: thermal conductivity $K$, bulk density
$\rho$, and specific heat $c_p$. We adopt the functional forms of
\citet{hayne2017}, derived from orbital infrared observations. The
key one is the conductivity:
\[
K(T,z) = \underbrace{\Big[K_s + (K_d-K_s)\big(1-e^{-z/H}\big)\Big]}
         _{\text{structural part: depends on depth}}
       \times
         \underbrace{\Big[1 + \chi\,(T/T_\mathrm{ref})^3\Big]}
         _{\text{radiative part: depends on temperature}}.
\]
Read it left to right. The structural bracket says conductivity
climbs smoothly from a surface value $K_s$ to a deep value $K_d$,
with the transition happening over an e-folding depth $H\approx 6$\,cm
--- so by $\sim\!20$\,cm the regolith has essentially reached $K_d$.
The radiative bracket adds a temperature-dependent boost: at the
microscopic scale, heat also crosses the gaps between grains by
infrared radiation, and that pathway strengthens as $T^3$. The
coefficient $\chi$ sets how strong that boost is.

\begin{description}
\item[$K_s$] --- surface conductivity, $\approx 7.4\times10^{-4}$\,W\,m$^{-1}$\,K$^{-1}$.
  The fluffy top skin; an extremely good insulator.
\item[$K_d$] --- deep conductivity. \emph{This is what we retrieve.}
\item[$H$] --- e-folding depth of the $K_s\!\to\!K_d$ transition,
  $\approx 6$\,cm.
\item[$\chi$] --- radiative-boost coefficient, $\approx 2.7$.
\item[$T_\mathrm{ref}$] --- the temperature at which the radiative
  term is normalised, $350$\,K.
\item[$\rho(z)$] --- density, rising from $\sim\!1100$ to
  $\sim\!1800$\,kg\,m$^{-3}$ with depth as overburden compacts the
  regolith.
\item[$c_p(T)$] --- specific heat, a polynomial in temperature.
\end{description}

The crucial design choice of this study: we hold \emph{every one of
these except $K_d$} fixed at its published value. The retrieval has
exactly one free knob. That is what makes the result interpretable
--- if the model fits better, it is because $K_d$ moved, not because
five parameters were quietly co-tuned.

\begin{aside}[Why fix everything but $K_d$?]
A model with many free parameters can fit almost anything, which
makes a good fit meaningless. By freezing the entire \citet{hayne2017}
recipe and varying only $K_d$, we ask a sharp, falsifiable question:
\emph{is there a single $K_d$ that reconciles the model with each
site's data?} If the answer differs between sites, that difference
cannot have been manufactured by tuning --- there was nothing else
to tune.
\end{aside}

\section{Step 3 --- Solving the equation on a computer}

The heat equation cannot be solved with pen and paper for a
realistic regolith, so we solve it numerically (Chapter~\ref{ch:numerics}).
Three ideas matter:

\begin{description}
\item[Geometric depth grid.] We chop the 5\,m column into layers
  that are very thin near the surface (millimetres, where
  temperature changes fast) and progressively thicker with depth
  (where it changes slowly). A uniform grid would either waste
  effort at depth or under-resolve the surface.
\item[Crank--Nicolson time stepping.] This is a stable, accurate
  recipe for advancing temperature one time step at a time. It
  averages the ``explicit'' and ``implicit'' updates, which keeps it
  from blowing up even at large time steps.
\item[Spin-up.] We do not know the true starting temperature
  profile, so we start from a rough guess and run the model through
  many lunar days until it forgets that guess and settles into a
  repeating annual cycle. We run 30 lunations and check that the
  temperature change between successive cycles has fallen below
  $5\times10^{-2}$\,K --- at that point the profile is converged.
\end{description}

\section{Step 4 --- Comparing model to data: the stability window}

The Apollo thermometers ran from 1971 to 1977, but the record is
not uniformly usable: there were instrument outages, a long
post-emplacement transient while drilling heat dissipated, and
power-conservation gaps (all visible in Fig.~1 of the Letter). For
each sensor we therefore select a \emph{stability window}: a
multi-lunation stretch where the long-term temperature drift is
negligible, so the window-mean temperature is a clean estimate of
the true equilibrium value $T_\mathrm{eq}$.

We also exclude every sensor shallower than 80\,cm --- the
\emph{borestem cut} (\S\ref{sec:borestem-diag}). The fibreglass
tube the thermometers sit in conducts heat along its own length,
short-circuiting the shallow regolith signal. The cut is decided
\emph{before} looking at the model fit, from an independent
diagnostic (the diurnal-amplitude profile), so it cannot bias the
retrieval.

\section{Step 5 --- The retrieval: finding the best $K_d$}

Now the actual measurement. For each site we run the forward model
many times, sweeping $K_d$ across a wide grid (1.5--15\,mW\,m$^{-1}$\,K$^{-1}$
at A15, 3--25 at A17). For each $K_d$ we compute the
root-mean-square error (RMSE) between the modelled deep-sensor
temperatures and the observed $T_\mathrm{eq}$ values. Plotting RMSE
against $K_d$ gives a curve with a clear minimum (Letter Fig.~5);
the $K_d$ at that minimum is the retrieved value, $K_d^{*}$.

\begin{key}[The headline result]
The two sites need different conductivities:
$K_d^{*}\approx4.86$\,mW\,m$^{-1}$\,K$^{-1}$ at Apollo~15 and
$\approx11.23$ at Apollo~17 --- a factor of $\sim\!2.3$ apart.
\end{key}

\section{Step 6 --- How sure are we? Two uncertainty treatments}

A best-fit number without an uncertainty is not a measurement.
We attach uncertainty two complementary ways.

\paragraph{The bootstrap.} We ask: \emph{how much would $K_d^{*}$
move if we had happened to sample a slightly different set of
sensors, sitting at slightly different depths?} We resample the
deep-sensor set with replacement, jitter each depth by the
documented $\pm2.5$\,cm placement uncertainty, and recompute
$K_d^{*}$ --- 1500 times. The spread of those 1500 values is the
bootstrap confidence interval (Letter Fig.~6). It answers ``how
stable is the answer against the data we happened to collect?''

\paragraph{The Bayesian posterior.} The basal flux $Q_b$ is itself
uncertain. Because the steady-state temperature gradient depends
only on the \emph{ratio} $Q_b/K_d$, a single borehole cannot
separate the two --- the \emph{$K_d/Q_b$ degeneracy}
(\S\ref{sec:posterior}). The Bayesian treatment confronts this
head-on: it puts a prior on $Q_b$ from the published reanalyses and
maps out the joint probability of every $(K_d, Q_b)$ pair using
MCMC. The result is a posterior that is honestly wider than the
bootstrap interval, because it now also carries the $Q_b$
uncertainty.

\begin{aside}[Why two methods and not one?]
They answer different questions. The bootstrap treats $Q_b$ as a
fixed input and reports the uncertainty from the data sample alone.
The Bayesian posterior treats $Q_b$ as itself uncertain. Reporting
both is honest: it shows the reader exactly which part of the
uncertainty comes from the sensors and which from the basal-flux
debate. Reassuringly, the \emph{inter-site contrast} survives in
both treatments, because rescaling $Q_b$ moves both sites together
and cannot erase the difference between them.
\end{aside}

\section{Step 7 --- Checking the answer against data it never saw}

A fit can look good and still be wrong. We test the retrieved
$K_d^{*}$ on three independent pieces of evidence it was not fitted
to:

\begin{enumerate}
\item \textbf{Diviner surface closure} --- the model run at the
  retrieved $K_d^{*}$ reproduces the orbital Diviner surface-
  temperature curve (terminator timing, plateau, night floor).
\item \textbf{Sensor-type cross-prediction} --- fit $K_d$ using only
  one type of thermometer, predict the other type; the prediction
  holds.
\item \textbf{Leave-one-deepest-out} --- drop the deepest sensor,
  refit, and check it predicts the held-out temperature.
\end{enumerate}

All three pass, which is what lets us trust the retrieval rather
than just report it.

\section{Where to go from here}

You now have the whole arc. The remaining chapters supply the
detail: Chapter~\ref{ch:moon} on lunar surface temperatures and the
thermal skin depth; Chapter~\ref{ch:pde} on the heat equation and
boundary conditions; Chapter~\ref{ch:hayne} on the
\citet{hayne2017} parameterisation and the two landing sites;
Chapter~\ref{ch:numerics} on the solver; Chapters~\ref{ch:validation}--\ref{ch:crosschecks}
on the validation and cross-checks; and Chapter~\ref{ch:repro} on
reproducing every number from the code.
...
the skin temperature, and $F_\odot(t)$ is the instantaneous solar
irradiance incident on the surface, given by
(\ref{eq:insolation-rep}) below. (On an airless body there is no
atmosphere to attenuate sunlight, so the irradiance reaching the
ground equals the irradiance at the top of the column --- there is
no separate ``top-of-atmosphere'' value as there would be on
Earth.)
...
\subsection*{A non-specialist primer: how to read this section}

This section reports a \emph{Bayesian posterior probability density}
on the two parameters that jointly govern the deep equilibrium
temperature, the deep regolith conductivity $K_d$ and the basal
heat flux $Q_b$. A few definitions for readers without a
statistics background:

\begin{aside}[Posterior, in plain language]
The \emph{posterior} $P(K_d, Q_b \mid \mathrm{data})$ is the
probability that the true values of $(K_d, Q_b)$ lie in any small
region of the $(K_d, Q_b)$ plane, given everything we know: the
Apollo HFE temperatures, the published prior expectation on $Q_b$
from the Saito reanalysis, and the physics of conductive heat flow
in the regolith. A 2-D posterior plot looks like a contour map of a
mountain: the peak is the most-probable parameter combination; the
inner contour encloses 32\% of the total probability ("$1\sigma$");
the second contour encloses 68\% ("$2\sigma$"); the outer encloses
95\%.
\end{aside}

\begin{aside}[MCMC: how we sample the posterior]
Computing the posterior on a fine grid would require evaluating the
likelihood at every $(K_d, Q_b)$ point, which is too expensive when
the grid is large. \emph{Markov-Chain Monte Carlo} (MCMC) instead
draws \emph{samples} from the posterior, with each sample more
likely to land in high-probability regions than in low-probability
regions. Many samples together approximate the full posterior shape.
We use the affine-invariant ensemble sampler of
\citet{foreman_mackey2013emcee}, which runs 32
``walkers'' in parallel and exchanges information between them. The
walkers explore the parameter space for 4000 steps; the first 1000
are discarded as ``burn-in'' (the walkers haven't yet found the
high-probability region). The remaining $32 \times 3000 \approx
10^5$ samples are what define the posterior in
Fig.~\ref{fig:posterior}.
\end{aside}

\begin{aside}[Why the posterior contours are tilted]
The deep equilibrium temperature gradient is $|\partial_z T| =
Q_b / K_d$. If we double both $K_d$ and $Q_b$, the gradient is
unchanged, so the deep temperatures predicted by the model are also
unchanged --- our data cannot distinguish between
$(K_d, Q_b)$ and $(\alpha K_d, \alpha Q_b)$ for any positive
$\alpha$. This is called the \emph{$K_d/Q_b$ degeneracy}, and it
makes the posterior contours diagonal stripes (along ``rays''
emanating from the origin) rather than circular blobs. The dotted
lines in Fig.~\ref{fig:posterior}(a)(b) are explicit iso-ratio
rays at $Q_b/K_d = 1, 2, 3$\,K\,m$^{-1}$ for reference.
\end{aside}

\begin{aside}[How a Gaussian prior on $Q_b$ ``breaks the degeneracy'']
Before looking at any HFE data, we already have an independent
estimate of the basal heat flux from the surface heat-flow
reductions of \citet{langseth1976} and \citet{saito2007,nagihara2018}.
That prior knowledge can be encoded as a Gaussian-shaped probability
distribution on $Q_b$ (mean and standard deviation listed below).
Multiplying the prior into the likelihood narrows the posterior in
the $Q_b$ direction, which in turn picks out a finite range along
the $K_d/Q_b$ ridge. Without the prior, the posterior would be
infinitely elongated; with it, the posterior is a finite, slanted
ellipse.
\end{aside}

\subsection*{Methodology and posterior summary}
...
ensemble MCMC sampler of \citet{foreman_mackey2013emcee}: 32
walkers, 4000 steps, the first
...
\caption{\textbf{SI Fig.~S11 --- Bayesian joint $(K_d, Q_b)$ posterior
from MCMC at the two Apollo sites.} Panels (a)--(b):
...
lower $Q_b$. This is the figure cited in the main text as
``Fig.~S11''; it has been relocated from the letter to the SI to
keep the main text focused on the headline contrast.}
\label{fig:posterior}
\end{figure}
...
\caption{\textbf{Direct two-site posterior comparison and contrast
distribution.} (a) Apollo~15 (teal) and Apollo~17 (coral) marginal
$P(K_d\,|\,\mathrm{data})$ overlaid as Gaussian-KDE estimates with
median lines and 16--84 percentile bars at the foot of each curve.
The annotated probability $P(K_d^{\rm A17}>K_d^{\rm A15})$ is the
fraction of paired posterior draws for which the A17 sample exceeds
the A15 sample, i.e.\ a distribution-level statement of how strongly
the data discriminate the two sites. (b) Posterior of the inter-site
contrast $\Delta K_d \equiv K_d^{\rm A17}-K_d^{\rm A15}$ with 66\%
(darker) and 95\% (lighter) credible intervals shaded; the dashed
line at $\Delta K_d=0$ marks the null hypothesis of a uniform deep
conductivity. This is the Bayesian counterpart of the bootstrap
contrast shown in the main letter.}
...
temperature is $12.4$\,K at A15 and $17.5$\,K at A17, with a model
warm-bias of $+4.9$ and $+9.1$\,K respectively
...
\caption{\textbf{SI Fig.~S12 --- Surface-temperature closure against
the Diviner GCP \citep{williams2017diviner}.}
(a)--(b) Full diurnal cycle at each site: dots are the
...
absolute surface temperature. This is the figure cited in the main
text as ``Fig.~S12''; it has been relocated from the letter to the
SI to keep the main text focused on the headline contrast.}
