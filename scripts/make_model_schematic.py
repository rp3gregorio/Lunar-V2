...
gs = fig.add_gridspec(2, 1, height_ratios=[1.0, 1.0],
                      left=0.10, right=0.96, bottom=0.07, top=0.95, hspace=0.32)
...
ax0.text(hx - 1.35, 15, "Depth (cm)", ha="center", va="center",
         fontsize=10, color="0.30", rotation=90)

# surface line
ax0.plot([hx - 0.3, mx + col_w + 0.3], [0, 0], color="0.30", lw=1.2)
ax0.text((hx + mx + col_w) / 2, -1.5, "surface  ($z = 0$)",
         ha="center", va="bottom", fontsize=9, color="0.30", style="italic")
...
ax0.text(hx_c, -5.0, "Hayne (2017)", ha="center", va="bottom",
         fontsize=11, fontweight="bold", color=C_H)
ax0.text(hx_c, -3.7, "smooth exponential",
         ha="center", va="bottom", fontsize=8.5, color=C_H, style="italic")

# K_s box (top of column)
ax0.text(hx_c, 3.0,
         fr"$K_s = {Ks_H*1e3:.2f}$" "\n" r"mW m$^{-1}$ K$^{-1}$",
         ha="center", va="center", fontsize=8.5, color=C_TXT,
         linespacing=1.3,
...
ax0.annotate(
    ...
    fontsize=8.5, color=C_H, ha="center", va="center",
    linespacing=1.3,
...
for (z0, z1, fc, lab, tc) in layers:
    ...
    ax0.text(mx_c, (z0 + z1) / 2, lab, ha="center", va="center",
             fontsize=9, color=tc, linespacing=1.3, fontweight="semibold")

# Header
ax0.text(mx_c, -5.0, "Martinez & Siegler (2021)", ha="center", va="bottom",
         fontsize=11, fontweight="bold", color=C_MS_D)
ax0.text(mx_c, -3.7, "piecewise 3-layer",
         ha="center", va="bottom", fontsize=8.5, color=C_MS_D, style="italic")

# K_s callout — RIGHT of M&S column at top (don't conflict with the headers)
ax0.annotate(
    fr"$K_s = {Ks_M*1e3:.1f}$" "\n" r"mW m$^{-1}$ K$^{-1}$",
    xy=(mx + col_w, 3.5), xytext=(mx + col_w + 1.3, 1.5),
    fontsize=8.5, color=C_MS_D, ha="center", va="center", linespacing=1.3,
...
for z_break, lab in [(7, "7 cm"), (20, "20 cm")]:
    ...
    ax1.text(13.6, z_break + 0.2, lab, fontsize=8,
...
ax1.legend(loc="lower right", fontsize=9, framealpha=0.96, edgecolor="0.75")
